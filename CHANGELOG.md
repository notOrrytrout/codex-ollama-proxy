...
- Added default-on, content-hashed persistence for GIF, JPEG, PNG, and WebP inline images on loopback upstreams, replacing historical pixels with reusable path references while keeping active image-turn pixels inline. The proxy uses a dedicated cache below `~/.codex/attachments`, validates image signatures, limits decoded images to 20 MiB, requires a stable session identifier, and expires caches after 30 unused days by default without touching unrelated Codex attachments. Use `--no-persist-images` to opt out.
