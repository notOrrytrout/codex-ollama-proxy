...
- Preserved adaptor launcher settings across `install` and `restart` with a private, non-secret runtime state file, including preset-free Chat Completions starts, custom adaptor ports, and explicit completion-model overrides, while keeping post-preset `proxy-models.toml` route overrides intact.
...
- Added default-on, content-hashed persistence for GIF, JPEG, PNG, and WebP inline images when image auto-routing is enabled on loopback upstreams, replacing historical pixels with reusable path references while keeping active image-turn pixels inline. Disabling image auto-routing also bypasses persistence so manual model selection retains the complete image history. The proxy uses a dedicated cache below `~/.codex/attachments`, validates image signatures, limits decoded images to 20 MiB, requires a stable session identifier, and expires caches after 30 unused days by default without touching unrelated Codex attachments. Use `--no-persist-images` to opt out independently.
